...
## TODO: Edit the ConvBlock Class to suit the discriminator architechture
## Convolution block
...
### TODO: 1) It can send an output of the initial blocks before growing
...
class Generator(tf.keras.models.Sequential):
    def __init__(self, latent_dim = 512, normalize_latents = True,  *args, **kwargs):
        ...
        self.initial_layers = tf.keras.Sequential(
            [
                PixelNorm(),
                tf.keras.layers.Conv2DTranspose(filters=num_filters(self.stage),
        ...
        self.add(tf.keras.layers.InputLayer([latent_dim]))
        self.add(self.initial)
        self.resolution: int = resolution_of_stage(self.stage)

    def call(self, inputs, training=None, mask=None):
        print("Call was called inside the Generator Class")
